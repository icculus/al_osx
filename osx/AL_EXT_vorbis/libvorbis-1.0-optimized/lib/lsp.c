...
void vorbis_lsp_to_curve(float *curve,int *map,int n,int ln,float *lsp,int m,
  ...
  while(i<n){
    int k=map[i];
    int qexp;
    register float p=.7071067812f;
    register float q=.7071067812f;
    register float w=vorbis_coslook(wdel*k);
    register float *ftmp=lsp;
    register int c=m>>1;

#if 0
    // !!! FIXME: The scalar version is faster?!
    // !!! FIXME: (If you can make this faster, break it off into a
    // !!! FIXME: seperate function.
    if ((_vorbis_has_vector_unit()) && (!UNALIGNED_PTR(ftmp))) {
      register vector float vLd;
      register vector float vW;
      register vector float zero = (vector float) vec_splat_u32(0);
      register vector float vAccum;
      union { vector float v; float f[4]; } conv;

      conv.f[0] = conv.f[1] = conv.f[2] = conv.f[3] = -w;
      vW = conv.v;
      conv.f[0] = conv.f[1] = conv.f[2] = conv.f[3] = 1.0f;
      vAccum = conv.v;
      ...
      while (c >= 2) {
        ...
        ftmp += 4;
        vLd = vec_add(vLd, vW);
        vAccum = vec_madd(vLd, vAccum, zero);
      }

      conv.v = vec_madd(vAccum, vec_sld(vAccum, vAccum, 8), zero);
      q *= conv.f[0];
      p *= conv.f[1];
    }
#endif
